 ...
 *
 *  A wherebear            — two-stage pipeline · Vertex AI · gemini-3.7-flash
 *  B whataisle-openrouter — whataisle final pipeline · OpenRouter · gemini-3.6-flash
 *  C whataisle-vertex     — whataisle pipeline · Vertex AI · gemini-3.7-flash
 */
...
const PARADIGMS: {
  ...
  },
  {
    ...
    sub: 'whataisle pipeline · Vertex AI',
    model: 'gemini-3.7-flash',
    color: '#3b82f6',
...
export default function ComparePage() {
  ...
  // prep, raw decode, preview, and both whataisle paradigms' single
  // row-detect calls start now, so the pipelines are ready to go the moment
  // Run is clicked.
  useEffect(() => {
    if (!file) return;
  ...
  return (
    <div style={{ padding: '20px 16px 60px', background: '#fff', color: '#111', minHeight: '100vh', maxWidth: 1400, margin: '0 auto' }}>
      ...
      <p style={{ fontSize: 13, color: '#555', margin: '0 0 16px', maxWidth: 900 }}>
        Upload one shelf photo — it runs through three scanning paradigms in parallel:{' '}
        <b style={{ color: '#10b981' }}>A</b>{' '}WhereBear&apos;s two-stage pipeline and{' '}
        <b style={{ color: '#3b82f6' }}>C</b>{' '}WhatAisle on Vertex use <b>gemini-3.7-flash</b>;
        <b style={{ color: '#8b5cf6' }}> B</b>{' '}keeps OpenRouter&apos;s independently pinned{' '}
        <b>gemini-3.6-flash</b>. Compare cost, wall time, detected products, and box overlays side by side.
      </p>
            ...
            <div key={p.key} style={{ border: `1px solid #e4e4e7`, borderTop: `3px solid ${p.color}`, borderRadius: 10, padding: 12, minWidth: 0 }}>
              <div style={{ marginBottom: 8 }}>
                ...
                <div style={{ fontSize: 11, color: '#71717a' }}>
                  {p.sub} · {s.state === 'done' ? s.result.model : p.model}
                </div>
              </div>
