...
export const STRINGS = {
  en: {
    ...
    find_guess_tap: 'Tap to see photo',
    find_guess_no_photo: 'No photo for this item yet',
    ...
    find_stale_hint: 'Greyed shelf: re-scanned since, item not seen — probably moved.',
    find_likely: 'Likely',
    ...
    admin_queue_title: 'Upload queue',
    queue_title: 'Upload queue',
    queue_active: (n: number) => `${n} uploading`,
    queue_failed: (n: number) => `${n} failed`,
    queue_saved: (n: number) => `${n} saved`,
    queue_empty: 'Queue is empty — snap a shelf and photos upload themselves.',
    queue_go_snap: 'Snap a shelf',
    queue_section_active: 'Uploading',
    queue_section_failed: 'Failed',
    queue_section_saved: 'Recently saved',
    queue_status_queued: 'Waiting',
    queue_status_detecting: 'Recognizing…',
    queue_status_detected: 'Recognized',
    queue_status_saving: 'Saving…',
    queue_status_saved: 'Saved',
    queue_retry: 'Retry',
    queue_delete: 'Delete',
    queue_clear_saved: 'Clear saved',
    queue_retrying: 'Auto-retrying soon',
    queue_attempts: (n: number, max: number) => `attempt ${n}/${max}`,
    queue_products_n: (n: number) => `${n} products`,
    queue_err_network: 'No connection — retries when back online',
    queue_err_too_large: 'Photo over 20 MB — delete and reshoot',
    queue_err_nothing_detected: 'Nothing detected in this photo',
    queue_err_unreadable: 'Photo could not be read — delete and pick it again',
    queue_err_server: 'Server error',
    queue_err_save: 'Save failed',
    queue_view_scan: 'Tap to see boxes & products',
    queue_no_boxes: 'No box overlay on this photo',
    snap_autosave: 'Photos save to shelf memory automatically — you can leave this page anytime.',
    snap_saved_n: (n: number) => `${n} saved`,
    snap_view_queue: 'View upload queue',
  ...
  zh: {
    ...
    find_stale_hint: '灰色货架：后来重拍时没再见到，可能已挪走。',
    find_likely: '应该是',
    find_locating: '正在找货架…',
    ...
    admin_queue_title: '上传队列',
    queue_title: '上传队列',
    queue_active: (n: number) => `${n} 张处理中`,
    queue_failed: (n: number) => `${n} 张失败`,
    queue_saved: (n: number) => `已入库 ${n} 张`,
    queue_empty: '队列是空的——扫货架后照片会自动上传入库。',
    queue_go_snap: '去扫货架',
    queue_section_active: '处理中',
    queue_section_failed: '失败',
    queue_section_saved: '最近入库',
    queue_status_queued: '排队中',
    queue_status_detecting: '识别中…',
    queue_status_detected: '已识别',
    queue_status_saving: '入库中…',
    queue_status_saved: '已入库',
    queue_retry: '重试',
    queue_delete: '删除',
    queue_clear_saved: '清除已入库',
    queue_retrying: '稍后自动重试',
    queue_attempts: (n: number, max: number) => `第 ${n}/${max} 次尝试`,
    queue_products_n: (n: number) => `${n} 个商品`,
    queue_err_network: '网络中断——恢复后自动重试',
    queue_err_too_large: '照片超过 20MB——请删除后重拍',
    queue_err_nothing_detected: '这张没识别到商品',
    queue_err_unreadable: '照片读不出来——请删除后重新选',
    queue_err_server: '服务器出错',
    queue_err_save: '入库失败',
    queue_view_scan: '点开看框选和识别结果',
    queue_no_boxes: '这张没有框选数据',
    snap_autosave: '照片会自动识别并入库——随时可以离开本页。',
    snap_saved_n: (n: number) => `已入库 ${n} 张`,
    snap_view_queue: '查看上传队列',
    passcode_title: '店员入口',
    passcode_hint: '请输入密码',
    passcode_cancel: '取消',
