...
// `global` serves Gemini 3.7 Flash and is also required by the shared Flex
// transport. Keep it explicit so a regional GOOGLE_CLOUD_LOCATION cannot split
// the app's standard and Flex traffic across incompatible endpoints.
...
export const VISION_MODEL = process.env.GEMINI_MODEL || DEFAULT_GEMINI_MODEL;
 ...
 * Requests also get a 60 s default deadline so a parked transport cannot hold
 * a process-wide concurrency slot forever. Callers with their own timeout or
 * AbortSignal keep that budget unchanged.
 *
...
type GenerateContentParams = Parameters<typeof genai.models.generateContent>[0];

function withDefaultTimeout(
  params: GenerateContentParams,
  timeoutMs: number,
): GenerateContentParams {
  // A caller-provided AbortSignal or HTTP timeout owns its request budget.
  // In particular, the scan transports use longer image-generation budgets.
  if (params.config?.abortSignal || params.config?.httpOptions?.timeout !== undefined) {
    return params;
  }
...
export async function generateContentWithRetry(
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    try {
      ...
      try {
        return await genai.models.generateContent(withDefaultTimeout(params, attemptTimeoutMs));
      } finally {
        releaseGeminiSlot();
      }
    } catch (err) {
 ...
 * request without a 429. If the first attempt hasn't answered within
 * `hedgeAfterMs`, fire a duplicate and take the first successful result. Each
 * attempt has a finite deadline. Only use on cheap calls — it can double the
 * token spend of that call.
 */
export async function generateContentWithHedge(
  ...
  const first = generateContentWithRetry(params, attemptTimeoutMs);
  ...
  const second = generateContentWithRetry(params, attemptTimeoutMs);
  // Take the first SUCCESS. Promise.race would fail the whole operation if a
  // timed-out request rejected just before its healthy twin returned.
...
export async function transcribeAudio(
  ...
  const result = await generateContentWithHedge({
    ...
    config: {
      ...temperatureConfigForModel(VISION_MODEL, 0.2),
      thinkingConfig: { thinkingLevel: lowestSupportedThinkingLevel(VISION_MODEL) },
    },
  }, 4000);
...
async function stage1DetectBoxes(
  ...
  const result = await generateContentWithHedge({
    ...
    config: {
      responseMimeType: 'application/json',
      ...temperatureConfigForModel(model, 0.1),
      // Stage 1 is pure perception (locate boxes) — no reasoning needed.
      // 3.7 no longer supports MINIMAL, so use its lowest valid level (LOW)
      // while retaining MINIMAL for an older model override.
...
async function stage2IdentifyBatch(
  ...
  const result = await generateContentWithHedge({
    ...
    config: {
      responseMimeType: 'application/json',
      ...temperatureConfigForModel(model, 0.2),
      // Stage 2 is visual recognition from close-up crops, not reasoning —
      // ask for the lowest thinking level that the selected model accepts.
      thinkingConfig: { thinkingLevel: lowestSupportedThinkingLevel(model) },
    },
  }, 15000, 60000);
...
export async function detectAndIdentifyProducts(
   ...
   *  its pinned Vertex model without touching the app's VISION_MODEL). */
  model: string = VISION_MODEL
