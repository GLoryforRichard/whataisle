 ...
 */
const ALIAS_MODEL = process.env.GEMINI_ALIAS_MODEL || DEFAULT_GEMINI_MODEL;
...
export async function execExpandAliases(args: { canonical_name: string; category?: string }) {
  const result = await generateContentWithRetry({
    ...
    config: {
      ...
      ...temperatureConfigForModel(VISION_MODEL, 0.4),
    },
  });
  try {
...
export async function execExpandAliasesBatch(args: {
  ...
  const result = await generate({
    ...
    config: {
      responseMimeType: 'application/json',
      ...temperatureConfigForModel(ALIAS_MODEL, 0.4),
    },
  });
  try {
    const parsed = JSON.parse(result.text ?? '{}');
