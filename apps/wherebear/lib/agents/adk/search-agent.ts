 ...
 *
 * Model + Vertex config mirror lib/gemini.ts exactly, including the shared
 * `global` endpoint used by standard and Flex traffic. Constructed lazily as a
 * singleton so importing this module never spawns the MCP subprocess on its own.
 */
...
export function getSearchAgent(): LlmAgent {
  ...
  _agent = new LlmAgent({
    ...
    generateContentConfig: {
      ...
      // costing seconds per agent turn. 3.7's lowest valid level is LOW;
      // older overrides retain MINIMAL through the compatibility helper.
