...
export async function synthesizeFinish(input: {
  ...
  if (pool.length > 0) {
    ...
    try {
      ...
      // Fail-open: retrieval already returned a useful ranked pool, so a slow
      // or unavailable synthesis model must not keep the customer waiting.
      const msg = err instanceof Error ? err.message : String(err);
      console.warn(`[search] answer synthesis failed; using deterministic fallback — ${msg.slice(0, 160)}`);
    }
  }
