 ...
 * - reasoningEffort 'off' asks for the selected model's lowest supported
 *   thinking level: LOW on Gemini 3.7, MINIMAL on older overrides. undefined
 *   leaves thinking dynamic (band detection needs the reasoning for grouping).
 */
...
export function buildVertexConfig(opts: CallModelOptions): Record<string, unknown> {
  const config: Record<string, unknown> = {
    ...temperatureConfigForModel(opts.modelId, 0),
    maxOutputTokens: 16_000,
    responseMimeType: 'application/json',
  };
  if (opts.schema) config.responseJsonSchema = opts.schema;
  if (opts.reasoningEffort) {
    const thinkingLevel = opts.reasoningEffort === 'off'
      ? lowestSupportedThinkingLevel(opts.modelId)
