...
> 最后核对：2026-07-25 · 分支 `fix/tenant-isolation-bypass`（16 个 commit 待合并）
>
...
| **平台后台**<br>§7 | tenants / costs / users / tickets / announcements / audit / onboarding / mapping | 🔴 **Cloud Scheduler 未开通**——任务队列代码已就绪，但定时触发要你跑 `infra/gcp/07-scheduler.sh`（需 gcloud 登录）。在那之前任务会排队等待，可用同一个 bearer 手动触发 |
...
| **测试与 CI** | **37 个 E2E（11 spec）** + 32 个单测<br>**审计闸已清零**（next 16.2.11 + 六组 override + shadcn 挪 devDeps；0 high）<br>PR 跑 audit + lint + typecheck + build<br>`e2e.yml` 独立 release 工作流<br>`AI_STUB` 让全链路可离线跑 | 🟡 16 个 dependabot PR 待重跑（闸已通，应能自行变绿）<br>⚪ E2E 结构性覆盖不到生产构建行为（`isE2ETestMode()` 硬要求 `NODE_ENV=development`） |
| **合规与法务** | 隐私 / 条款 / Cookie 中英双语<br>条款变更再确认流程<br>审计日志 | 🔴 **AI 供应商商务条款未核实**——现在是两家（DashScope + OpenRouter），要确认转售许可 + 店铺数据不被用于训练。真实店铺数据流入前必须先过 |
| **代码健康** | 死掉的 docs/blog/changelog 脚手架已清（-1689 行）<br>CLAUDE.md / AGENTS.md / README.md 已按代码实况重写 | ⚪ knip 仍报 41 个未用文件（多为图标与 data-table 碎片）；**未用依赖已清零** |
...
4. 🔴 **跑 `infra/gcp/07-scheduler.sh`** —— 任务队列的定时触发。脚本已写好且幂等，只需你 `gcloud auth login` 后执行一次。

**代码侧本轮已全部完成**（16 个 commit）：隔离绕过、CI 审计闸、开店状态机、扫描流程对齐、拆邀请制与 Creem、嵌入向量污染、成本记账补全、模拟登录一次性+可撤销、任务队列+可续传 re-embed、四项 E2E、依赖清理。
