...
docker buildx build --platform linux/amd64 \
  ...
  --build-arg NEXT_PUBLIC_STRIPE_PRICE_LIFETIME="$NEXT_PUBLIC_STRIPE_PRICE_LIFETIME" \
...
gcloud run deploy "$SERVICE_NAME" --region="$REGION" \
  ...
  --set-env-vars="NODE_ENV=production,MAIL_PROVIDER=resend,STORAGE_PROVIDER=gcs,STORAGE_BUCKET_NAME=${BUCKET_NAME},GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_LOCATION=${REGION},NEXT_PUBLIC_BASE_URL=${BASE_URL},NEXT_PUBLIC_ROOT_DOMAIN=${ROOT_DOMAIN},TRUST_GCP_LOAD_BALANCER=false,PUBLIC_SIGNUP_ENABLED=true,VIDEO_NOTIFY_EMAIL=lby2024xd@outlook.com" \
